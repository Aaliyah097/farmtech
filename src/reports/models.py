...
class BalanceSheetItems(models.Model):
    ...
    direction = models.BooleanField(verbose_name='Приход/расход',
...
class AccountingTransactions(models.Model):
    balance_sheet_item = models.ForeignKey(to=BalanceSheetItems,
                                           verbose_name='Статья',
                                           null=True,
                                           on_delete=models.SET_NULL)
    report = models.ForeignKey(to=FinancialReports,
                               verbose_name='Финанмовый отчет',
                               on_delete=models.CASCADE,
                               related_name='transactions',
                               blank=True)
    on_date = models.DateField(verbose_name='На дату',
                               default=now,
                               blank=True)
    ...
    def __str__(self):
        return f"Проводка {self.balance_sheet_item} на сумму {self.amount} от {self.on_date}"
